...
object ALSTrainer {
  ...
  def adjustALSParam(trainData: RDD[Rating], testData: RDD[Rating]): Unit ={
    val result: Array[(Int, Double, Double)] =
      for (rank <- Array(50, 100, 200, 300); lambda <- Array(0.01, 0.1, 1))
        yield {
    ...
    // 控制台打印输出最优参数
    println(result.minBy(_._3))   //或者 result.sortBy(_._3).head()
    //输出结果  (200,0.1,0.9145504070723784)
  }
